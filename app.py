...
# --- STILE GLOBALE ORIGINARIO (PULITO E MODERNO) ---
st.markdown(
    """
    <style>
        .stApp {
...
def renderizza_classifica_stile_card(torneo_selezionato, g_nome):
  ...
  for idx, (coppia, info) in enumerate(sorted_c):
    ...
    border_color = "#22c55e" if is_fascia_a else "#ef4444"
    bg_gradient = "linear-gradient(135deg, rgba(20, 83, 45, 0.4) 0%, rgba(15, 23, 42, 0.9) 100%)" if is_fascia_a else "linear-gradient(135deg, rgba(127, 29, 29, 0.4) 0%, rgba(15, 23, 42, 0.9) 100%)"
    shadow_color = "rgba(34, 197, 94, 0.15)" if is_fascia_a else "rgba(239, 68, 68, 0.15)"
    dot_color = "#22c55e" if is_fascia_a else "#ef4444"
    ...
    st.markdown(
        ...
        <div style="background: {bg_gradient}; border: 1.5px solid {border_color}; border-radius: 12px; padding: 12px 16px; margin-bottom: 10px; box-shadow: 0 4px 12px {shadow_color}; display: flex; align-items: center; justify-content: space-between;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <span style="font-size: 15px; font-weight: bold; color: #f8fafc;">⚽🏆 {coppia}</span>
            </div>
            <div style="display: flex; gap: 14px; text-align: right; font-size: 13px;">
                <div>
                    ...
                    <span style="font-weight: 800; color: #fbbf24; font-size: 15px;">{info['punti']}</span>
                </div>
                <div>
                ...
                <div>
                    ...
                    <span style="color: {"#22c55e" if info['dr'] >= 0 else "#ef4444"}; font-weight: 600;">{info['dr']:+d}</span>
                </div>
            </div>
        </div>
        """,
...
st.markdown(
    ...
    <div style="text-align: left; margin-bottom: 8px;">
        <span style="color: #38bdf8; font-size: 13px; letter-spacing: 2px; font-weight: 800;">TOURNAMENT CIRCUIT SELECTION</span>
        <h1 style="font-size: 28px; margin: 4px 0 12px 0; color: #f8fafc; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
  ...
  with col_tit_vista:
    ...
    if not t_data["coppie"]:
      ...
      for idx, c in enumerate(t_data["coppie"], 1):
        ...
        with col_ic1:
          st.markdown(f"<div style='padding: 6px 10px; background: rgba(30, 41, 59, 0.9); border: 1px solid #38bdf8; border-radius: 8px; margin-bottom: 5px; font-size: 14px; color: #f8fafc;'><b>{idx}.</b> ⚽ {c}</div>", unsafe_allow_html=True)
        with col_ic2:
  ...
  with col_cod_vista:
    ...
    if not t_data["coda"]:
      ...
      for idx_c, c_coda in enumerate(t_data["coda"], 1):
        ...
        with col_cc1:
          st.markdown(f"<div style='padding: 6px 10px; background: rgba(120, 53, 15, 0.4); border: 1px solid #f59e0b; border-radius: 8px; margin-bottom: 5px; font-size: 14px; color: #fef3c7;'><b>{idx_c}.</b> ⏳ {c_coda}</div>", unsafe_allow_html=True)
        with col_cc2:
  ...
  with st.expander(f"👁️ Segui la tua coppia: {coppia_selezionata}", expanded=True):
    ...
    st.markdown(
        ...
        <div class="cyber-card" style="border-color: #38bdf8; text-align: left; padding: 20px;">
            <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #38bdf8; font-weight: bold; margin-bottom: 2px;">LA TUA COPPIA</div>
            <div style="font-size: 22px; font-weight: 800; color: #f8fafc; margin-bottom: 14px;">🤝 {coppia_selezionata}</div>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <div style="background: rgba(30, 41, 59, 0.9); border: 1px solid #38bdf8; border-radius: 10px; padding: 10px; flex: 1; min-width: 100px; text-align: center;">
                ...
                <div style="background: rgba(30, 41, 59, 0.9); border: 1px solid #38bdf8; border-radius: 10px; padding: 10px; flex: 1; min-width: 100px; text-align: center;">
                    ...
                    <div style="font-size: 16px; font-weight: 700; color: #38bdf8; margin-top: 2px;">{girone_mio if girone_mio else 'N.D.'}</div>
                </div>
                <div style="background: rgba(30, 41, 59, 0.9); border: 1px solid #38bdf8; border-radius: 10px; padding: 10px; flex: 1; min-width: 100px; text-align: center;">
      ...
      for m in partite_in_corso:
        ...
        st.markdown(
            ...
            <div class="match-live-card" style="margin-bottom: 12px;">
                <div style="font-size: 14px; color: #fbbf24; font-weight: bold; margin-bottom: 8px;">{tavolo_str}</div>
                <div style="font-size: 16px; font-weight: bold; color: #f8fafc;">🤝 {m['c1']}</div>
                <div style="margin: 4px 0; font-size: 12px; font-weight: bold; color: #94a3b8;">VS</div>
                <div style="font-size: 16px; font-weight: bold; color: #f8fafc;">🤝 {m['c2']}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
  ...
  with col_coda:
    ...
    if not partite_in_coda_correnti:
      ...
      for idx, m in enumerate(partite_in_coda_correnti):
        st.markdown(
            f"""
            <div style="background: rgba(30, 41, 59, 0.9); border: 1.5px solid #22c55e; padding: 14px; border-radius: 10px; margin-bottom: 10px; color: #4ade80; text-align: center; box-shadow: 0 4px 10px rgba(34, 197, 94, 0.1);">
                <b style="font-size: 13px;">⏳ {idx+1}. {m['girone']}</b><br>
                <b style="color: #f8fafc; font-size: 14px;">{m['c1']} vs {m['c2']}</b>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if campione:
      st.markdown(
          ...
          <div class="cyber-card-gold" style="padding: 20px; margin-top: 15px;">
              ...
              <p style="font-size: 16px; color: #fef3c7;">🥉 3° POSTO: {terzo_posto if terzo_posto else 'N.D.'}</p>
          </div>
          """,
          unsafe_allow_html=True,
      )
